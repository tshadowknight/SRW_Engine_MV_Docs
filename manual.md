...
* In the output folder open package.json and replace is with the following:
...
\<pilotTags: newtype,leader\> Freeform tags applied to the pilot separated by commas. Can be used for battle text or for applying abilities.<br>
...
Up to 200 entries can be listed.<br>
...
\<mechAttribute1:vaccine\>The primary attribute of the mech, used if ENABLE\_ATTRIBUTE\_SYSTEM is on.<br>
\<mechAttribute2:fire\>The secondary attribute of the mech, used if ENABLE\_ATTRIBUTE\_SYSTEM is on.<br>
...
\<mechTags: real,gundam\> Freeform tags applied to the mech separated by commas. Can be used for battle text or for applying abilities.<br>
...
\<mechTransformsWhenTargetPresent: 1, 23\> The first parameter is the id of the mech that should be present on the stage, the second id is the id of the mech into which this mech will transform if the condition is met.<br>
**This property should not be used on Battle Ship units!**<br>

\<mechTransformsWhenTargetMissing: 1, 22\> The first parameter is the id of the mech that should be missing on the stage, the second id is the id of the mech into which this mech will transform if the condition is met.<br>
**This property should not be used on Battle Ship units!**<br>
...
\<mechMenuSprite:harold\>  The name of a .png file in the img/menu folder. This image is displayed in the menus when the full size mech sprite is shown. If omitted the main.png sprite of the unit will be used instead.<br>
...
\<mechSpawnAnimSoundEffect:SRWAppear\> The sound effect that plays when spawning the unit<br>

### Destroy Animation

The listed values are the defaults

\<mechDestroyAnimName:Explosion1\> The name of the sprite sheet found in img/animations<br>
\<mechDestroyAnimFrameSize:192\> The width and height of the frames of the animation<br>
\<mechDestroyAnimSheetWidth:5\> The width in frames of the sprite sheet<br>
\<mechDestroyAnimFrames:8\> The number of animation frames to use from the sprite sheet<br>
\<mechDestroyAnimSpeed:2\> The speed of the animation in frames spent for each frame of the animation<br>
\<mechDestroyAnimDisappearFrame:3\> The frame on which the unit that is being destroyed will become visible<br>
\<mechDestroyAnimSoundEffect:SRWExplosion\> The sound effect that plays when destroying the unit<br>
...
\<weaponAttribute1:holy\>Used if ENABLE\_ATTRIBUTE\_SYSTEM is on. If defined it will be used instead of the attacking mechs primary attribute.<br>
\<weaponAttribute2:holy\>Used if ENABLE\_ATTRIBUTE\_SYSTEM is on. If defined it will be used instead of the attacking mechs secondary attribute.<br>
...
\<weaponIsCounterOnly:1\> If set to 1 the weapon can only be used to counter attack. <br>
